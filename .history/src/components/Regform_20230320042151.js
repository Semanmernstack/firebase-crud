 ...
 function Regform ({  age, firstName, id, lastName }) {
  ...
  return (
    <div className="flex flex-col border p-4 gap-3 my-2 shadow-xl rounded-2xl w-2/3">
      ...
      <div>
        <input onChange={() => setUpdateName(e.target.value)} value={updateName} type="text" placeholder='updateName' />
        <input onChange={() => setUpdateLastname(e.target.value)} value={updateLastname} type="text" placeholder='updateLastname' />
        <input onChange={() => setUpdateNumber(e.target.value)} value={updateNumber} type="number" placeholder='updateNumber'/>
